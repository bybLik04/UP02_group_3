...
def handle_plot_request():
    ...
    start_length = float(bottle.request.forms.get('start_length'))
    end_length = float(bottle.request.forms.get('end_length'))
    ...
    if function == 'linear':
        ...
        x = np.linspace(start_length, end_length, 100)
        y = k * x + b
        ax.plot(x, y)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
    ...
    return template('plotting', image_data="static\images\graph1.png", title='Построение графиков', year=datetime.now().year)